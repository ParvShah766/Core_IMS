...
class ProductService extends ChangeNotifier {
  ...
  Future<void> loadFromBackend() async {
    try {
      final http.Response response = await http.get(_uri('/api/products'));
      if (response.statusCode < 200 || response.statusCode >= 300) {
        return;
      }

      final List<dynamic> decoded = jsonDecode(response.body) as List<dynamic>;
      final List<Product> remoteProducts = decoded
  ...
  void createProduct(Product product) {
    ...
    notifyListeners();
    unawaited(_upsertRemoteProduct(product));
  }
  ...
  Future<void> _upsertRemoteProduct(Product product) async {
    try {
      final Map<String, dynamic> body = _toJson(product);
      await http.post(
        _uri('/api/products'),
      ...
      // Local state remains source of truth if backend call fails.
    }
  }

  Product _fromJson(Map<String, dynamic> json) {
    final Map<String, dynamic> stockMap =
        (json['stockByLocation'] as Map<String, dynamic>? ??
        <String, dynamic>{});
    ...
    return Product(
      ...
      unitOfMeasure: (json['unitOfMeasure'] as String? ?? 'Units').trim(),
      reorderPoint: (json['reorderPoint'] as num? ?? 0).toInt(),
      reorderQuantity: (json['reorderQuantity'] as num? ?? 0).toInt(),
      stockByLocation: <String, int>{
        for (final MapEntry<String, dynamic> entry in stockMap.entries)
          entry.key: (entry.value as num? ?? 0).toInt(),
      },
    );
  }
