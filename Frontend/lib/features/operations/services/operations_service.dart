...
class OperationsService extends ChangeNotifier {
  ...
  Future<void> syncReceiptToBackend(ReceiptRecord receipt) async {
    try {
      final List<Map<String, dynamic>> items = receipt.lines
          .map(
            (OperationLine line) => <String, dynamic>{
              'skuCode': line.productCode,
      ...
      await http.post(
        _uri('/api/stock-movements/receipt'),
        headers: <String, String>{'Content-Type': 'application/json'},
        body: jsonEncode(<String, dynamic>{
          'warehouseId': 'default',
          'supplier': receipt.supplier,
          'location': receipt.lines.isNotEmpty ? receipt.lines[0].location : '',
          'items': items,
        }),
      );
    } catch (_) {
      // Silently fail - data is still in local storage
    }
  }

  Future<void> syncDeliveryToBackend(DeliveryOrderRecord delivery) async {
    try {
      final List<Map<String, dynamic>> items = delivery.lines
          .map(
            (OperationLine line) => <String, dynamic>{
              'skuCode': line.productCode,
              'quantity': line.quantity,
      ...
      await http.post(
        _uri('/api/stock-movements/delivery'),
        headers: <String, String>{'Content-Type': 'application/json'},
        body: jsonEncode(<String, dynamic>{
          'warehouseId': 'default',
          'customer': delivery.customer,
          'location': delivery.lines.isNotEmpty
              ? delivery.lines[0].location
              : '',
          'items': items,
        }),
      );
